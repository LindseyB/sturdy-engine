...
class Gifs {
  ...
  generateGif = (
    ...
    execSync(
      `${ffmpeg} -y -ss ${startTime} -t ${durationTime} -i "${videoFile}" -i palette.png -filter_complex "drawtext=text='${text}':x=(w-text_w)/2:y=(h-text_h)/2:fontsize=24:fontcolor=white:fontfile=${this.FONTFILE};[0:v][1:v] paletteuse" ${fileName}.gif`,
      (error, stdout, stderr) => {
        if (error) {
          console.log(`error: ${error.message}`)
          return
